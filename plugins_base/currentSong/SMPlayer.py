...
class SMPlayer( CurrentSong.CurrentSong ):

    def __init__( self ):
        CurrentSong.CurrentSong.__init__( self )
        self.isRunning()
        self.currentSong = ''
        self.template = "SMPlayer\\0Music\\01\\0{0}\\0%s\\0\\0"
        
    def isPlaying( self ):
        st = create_unicode_buffer(100)
        user.GetWindowTextW(self.smplayerwindow,st,100)
        return st.value != "SMPlayer"
        
    def isRunning( self ):
        global _window
        user.EnumWindows(EnumWindowsCallbackFunc,0)
        if _window is not None:
            self.smplayerwindow = _window
            return True
        else:
            return False
    ...
    def check( self ):
        ...
        if self.isPlaying():
            st = create_unicode_buffer(100)
            user.GetWindowTextW(self.smplayerwindow, st, 100)
            string = st.value.split(" - SMPlayer")[0]
